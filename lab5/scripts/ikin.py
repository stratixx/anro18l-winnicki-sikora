...
def find_t1(pairs, x, y):
	...
	for pair in pairs:
		try:
			...
			result1 = [t1c, pair[0], pair[1]]
			angles.append(result1)
			print(result1)
		except ValueError:
			pass
		...
		try:
			...
			result2 = [t1s, pair[0], pair[1]]
			angles.append(result2)
			print(result2)
		except ValueError:
			pass
		"""
...
def check_solution(angs, xz, yz, zz):
	results = []
	for ang in angs:
		robot = []
		...
		print(err)
		if err < 0.8:
			results.append([ang[0], ang[1], ang[2], err])
